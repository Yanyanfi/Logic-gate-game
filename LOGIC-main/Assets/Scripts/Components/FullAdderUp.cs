...
public class FullAdderUp : NewComponent
{
    ...
    public override void HandleInputs(object sender, EventArgs e)
    {
        ...
        int inputC=InputPins.GetValue(2);
        int sum = inputA + inputB + inputC % 2;
        OutputPins.SetValue(0, sum%256);
        OutputPins.SetValue(1, sum > 255 ? 1 : 0);
        OutputPins.SetValue(2, sum == 0 ? 1 : 0);
        OutputPins.SetValue(3, sum > 255 ? 1 : 0);
        OutputPins.SetValue(4, sum%256 > 127 ? 1 : 0);
    }
